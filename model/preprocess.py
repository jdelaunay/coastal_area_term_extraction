...
class Preprocessor:
    ...
    def preprocess(self, text, lang="en"):
        ...
        for s in doc.sents:
            ...
            sentence_list.append(
                (tokenized_text.split(), s)
            )  # Append tuple of tokens and original sentences
        ...
        for sen in sentence_list:
            ...
            for i in range(1, n + 1):  # 1-gram up to n-gram
                ...
                if i < len(s):
                    for (
                        n_gram
                    ) in n_grams:  # Look if n-grams are in the annotation dataset
    ...
    def map_data_label(self, gt_path, df_term_path, lang):
        if lang == "sl":
            df_term = pd.read_csv(
                df_term_path, header=None, delimiter="\t", names=["Term", "Label"]
            )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Map label from gold standard term list to corpus."
    )
    parser.add_argument(
        "--corpus_path",
        type=str,
        dest="corpus",
        default="./ACTER/en/corp/texts/annotated/",
    )
    parser.add_argument(
        "--term_path",
        type=str,
        dest="ann",
        default="..ACTER/en/corp/annotations/corp_en_terms_nes.ann",
    )
    parser.add_argument(
        "--output_csv_path",
        type=str,
        dest="output",
        default="./processed_data/ann/corp.pkl",
    )
